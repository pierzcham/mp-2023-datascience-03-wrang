...
Knit the R Markdown file `lab.Rmd` to md (github_document) document. Make sure it compiles without errors.
...
We'll use the **tidyverse** package for much of the data wrangling. Install the package with `install.packages` command if you haven't done it yet. You can load them by running the following in your Console.
...
```{marginfigure}
Note that we can achieve the same result using the `fct_other()` i.e. with `country_us = fct_other(country, "USA")`. If you type in the Console `?fct_other` and `?if_else`, you find out that the former comes from the forcast pacage and latter from the dplyr package.
```
